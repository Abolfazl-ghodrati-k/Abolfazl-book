...
export async function fetchRooms(token) {
  if (token) {
    ...
    await axios
      .get("url", {
        // @ts-expect-error TS(2345): Argument of type '{ token: any; }' is not assignab... Remove this comment to see the full error message
        token,
      })
      .then((res) => {
        if (res.data) {
        ...
        // @ts-expect-error TS(2339): Property 'error' does not exist on type 'AxiosResp... Remove this comment to see the full error message
